...
NOTE: Order matters. Use the reorder buttons to place your most specific audiences at the top. Teak evaluates tiers from top to bottom and stops at the first match — if a broad audience sits above a narrow one, players who qualify for the narrower tier will match the broader one first.
...
[TIP]
.Tell players exactly what they'll receive
====
When a player can see the specific reward waiting for them — rather than a vague promise — they're more likely to tap. Use a template tag to show each player their exact reward amount in the notification copy. This is especially valuable with tiered rewards, where VIP players can see the higher amount they'll actually receive.
====
...
All link features including expiration, claim limitation, and deep linking, are compatible with Tiered Rewards. Expiration and claim limits will apply across all reward tiers combined. For example, if you create a link with a Tiered Reward bundle and Limit Total Claims to 5 players, only 5 players will be able to claim any rewards from the link, regardless of selected reward tier.
...
NOTE: Editing a bundle after sending means any templated notification copy will no longer match the reward players actually receive. Double-check all Tiered Reward configurations before using them in a notification.
