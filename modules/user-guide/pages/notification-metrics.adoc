...
Clicks::
A click is tracked when a player enters the game through a link in an email or clicks on the link in an email using an Out of Game URL with the xref:user-guide::page$email.adoc#_skip_game_open[Skip Game Open, window=_blank] option enabled.
...
Returning Users is the count of distinct players each day who entered the game either directly through the email, or within an hour of clicking on the link in an email with the xref:user-guide::page$email.adoc#_skip_game_open[Skip Game Open, window=_blank] option enabled. +
