...
You can use the same Message features, like images, formatting, and rewards, with a Local Schedule. Additionally, a Message associated with a Local Schedule can template in custom per send tags provided by the game when it makes the appropriate call to send the notification (See xref:user-guide::custom-tags.adoc#_local_notification_tags[Local Notification Tags, window=_blank]). You will need to coordinate with your dev team to determine what additional data is available.
