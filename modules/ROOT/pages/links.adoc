...
When you create a Link, a URL is generated. This URL can have a xref:usage::page$rewards.adoc[Reward] attached and can be clicked by a max number of players -- which you specify.

== Link Creation

Links are a straightforward way to bring people into your game. Using Teak Links for existing players provides a great way to track and incentivize them. Creating links with or without xref:usage::page$rewards.adoc[Rewards] is quick and easy. Plus, you can even use Deep Linking to drop them wherever you want.
...
. If you want to incentivize the link, add a Reward Bundle (xref:usage::page$rewards.adoc#_bundle_creation[Creating Reward Bundles])
