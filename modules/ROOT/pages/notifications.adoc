...
Game Deep Link::
Direct players to a specific location in your game with a xref:unity::teak-unity-features.adoc#_deep_links[Deep Link, target=_blank] route. 
+
When the player taps the notification, the game will launch, but instead of beginning at the default screen, players begin their session in another location in the game, such as their inbox, a store, or a specific moment in their progression. Any Rewards will be applied as normal.
+
In order to use Deep Links, your team will need to integrate Teak's xref:unity::teak-unity-features.adoc#_deep_links[Deep Linking, target=_blank]. It's best practice to confirm with a developer what routes and values are available for use.

Out of Game URL::
Link to an out of game destination with a web URL (e.g. `https&#58;//example.com/`).
+
When the player taps the notification on **Android** or **iOS**, the game will launch, and then open the URL the device's default browser. Any attached Rewards will be granted to the player.
+
...
In order to use Deep Links, ask your team if they have integrated Teak's xref:unity::teak-unity-features.adoc#_deep_links[Deep Linking, target=_blank], and what routes and values are available for use.
...
Game Deep Link::
	Clicks will launch the game, any associated rewards will be granted, and the player will be taken to a specific section of the game. defained by a xref:unity::page$teak-unity-features.adoc#_deep_links[Deep Link Route, target=_blank].
